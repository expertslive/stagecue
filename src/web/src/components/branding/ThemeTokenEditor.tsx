  ...
  return (
    <div className="space-y-6">
      {grouped.map(({ group, tokens }) => (
        <section key={group}>
          <h3 className="text-sm font-semibold text-zinc-300 mb-3">{group}</h3>
          <ul className="space-y-2">
            {tokens.map((t) => {
              const current = overrides[t.key] ?? t.value;
              const isOverridden = overrides[t.key] !== undefined;
              return (
                <li key={t.key} className="flex items-start gap-3 rounded border border-white/10 bg-zinc-900/80 p-3">
                  <input
                    type="color"
                    value={current}
                  ...
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between">
                      ...
                      <input
                        ...
                        value={current}
                        onChange={(e) => setToken(t.key, e.target.value)}
                        className="w-28 rounded bg-zinc-950/60 border border-white/10 px-2 py-1 font-mono text-xs"
                      />
                    </div>
